...
def load_data():
    ...
    with open('../ComputerVisionFiles/fashion_mnist_classes.txt', 'r',
              encoding='utf-8') as f:
...
def benchmark_pca_components(embeddings, labels, class_names, n_components_list):
    ...
    for n_comp in n_components_list:
        ...
        if len(results) > 0:
            ...
            baseline_indices = {r['index'] for r in baseline_recs}
            current_indices = {r['index'] for r in current_recs}
...
def main():
    ...
    benchmark_pca_components(embeddings, labels, class_names, n_components_list)

    # Benchmark 2: Different index types
    benchmark_index_types(embeddings, labels, class_names)
